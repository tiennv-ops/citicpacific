  ...
  if ($('main.page-our-businesses').length > 0) {
    ...
    if (window.location.hash != '') {
      ...
      if ($('.main-not-show').length > 0) {
        var $targetSection = $('.section-scroll div').filter(function () {
          return $(this).attr('data-tab') === hashValue;
        });
        if ($targetSection.length > 0) {
          ourBussinessesMainTag.scrollLeft = $targetSection.offset().left;
        }
      ...
      window.requestAnimationFrame(function () {
        for (var i = 0; i < parallaxQuantityHorizontal; i++) {
          ...
          currentElementHorizontal.css({
            transform: "translate3d(" + scrolledHorizontal * 0.1 + "px ,0, 0)"
          });
        }
      });
    }
...
jQuery(window).on('load', function () {
  ...
  if (jQuery('main.page-our-businesses').length > 0) {
    ...
    jQuery('.page-template-template-our-businesses .next-prev .next').click(function (e) {
      jQuery('.page-template-template-our-businesses .page-our-businesses.section-scroll').addClass('scroll-smoth');
      let totalWidth = 0;
      var currentIndex = parseInt(jQuery(this).closest('.section').attr('data-tab-index'), 10);
      if (Number.isNaN(currentIndex)) {
        return false;
      }
      for (var i = 1; i <= currentIndex; i++) {
        totalWidth += jQuery('.page-template-template-our-businesses .page-our-businesses.section-scroll .section').filter(function () {
          return parseInt(jQuery(this).attr('data-tab-index'), 10) === i;
        }).outerWidth(true);
      }
    ...
    sectionIsInViewport();
    function isSafeTabValue(value) {
      return typeof value === 'string' && /^[A-Za-z0-9_-]+$/.test(value);
    }
    function getPtPageByTab(tabValue) {
      if (!isSafeTabValue(tabValue)) {
        return jQuery();
      }
      return jQuery('.pt-page').filter(function () {
        return jQuery(this).attr('data-tab') === tabValue;
      });
    }
    ...
    jQuery('.actions .action').on('click', function () {
      stopVideo();
      AOS.refreshHard();
      var dataTabPtPage = String(jQuery(this).closest('.section').attr('data-tab') || '');
      if (!isSafeTabValue(dataTabPtPage)) {
        return false;
      }
      var $targetPtPage = getPtPageByTab(dataTabPtPage);
      if (!$targetPtPage.length) {
        return false;
      }
      history.pushState('', '', window.location.origin + window.location.pathname + '#' + dataTabPtPage);
      jQuery('body').animate({ paddingTop: '0px' }).addClass('main-not-show');
      $targetPtPage.addClass('pt-page-current');
      $targetPtPage.addClass('pt-page-moveFromBottom');
      setTimeout(function () {
        $targetPtPage.removeClass('pt-page-moveFromBottom');
      }, 600);
