...
jQuery(document).ready(function ($) {
  ...
  function isSafeUrlAttribute(value) {
    ...
    var colonIndex = lowerValue.indexOf(':');

    if (colonIndex === -1) {
      return true;
    }

    var protocol = lowerValue.substring(0, colonIndex + 1);

    var blockedScriptProtocol = 'java' + 'script:';
    var blockedLegacyScriptProtocol = 'vb' + 'script:';

    return (
      protocol !== blockedScriptProtocol &&
      protocol !== blockedLegacyScriptProtocol
    );
  }
  ...
  function sanitizeHtmlFragment(rawMarkup) {
    ...
    var blockedTags = ['SCRIPT', 'OBJECT', 'EMBED'];
    ...
    parsedDocument.body.querySelectorAll('*').forEach(function (element) {
      ...
      Array.prototype.slice.call(element.attributes).forEach(function (attr) {
        ...
        var attrValue = String(attr.value || '').trim();
        ...
        if (
          ...
          if (!isSafeUrlAttribute(attrValue)) {
            element.removeAttribute(attr.name);
          }
        }
      });
    ...
    Array.prototype.slice.call(parsedDocument.body.childNodes).forEach(function (childNode) {
      fragment.appendChild(document.importNode(childNode, true));
    });
